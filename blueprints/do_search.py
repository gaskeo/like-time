...
def create_handler(api: InstApi, redis_api: Redis):
    ...
    def do_search():
        if redis_api.get(request.remote_addr):
            return jsonify({"answer": "error", "error-message": "много запросов шлешь, меня в инсте заблокируют... "
        ...
        if not posts:
            ...
            return jsonify({"answer": "error", "error-message": "посты не найдены, возможно аккаунт приватный"})
        post_links = []
